...
## Tong quan request
- Trinh duyet goi duong dan (vi du `/tasks/create`).
...
## Ban do file chinh
- Entry point & route map: [public/index.php](public/index.php) — khai bao route auth, tasks, lists, profile va goi `$router->dispatch()`.
- Core layer:
...
- Controllers:
  ...
  - [app/Controllers/TaskController.php](app/Controllers/TaskController.php) — trang chu tasks, filter inbox/my-day/important/planned/listId, CRUD, toggle complete/important, flash + redirect giu filter hien tai.
...
- Models:
  - [app/Models/Task.php](app/Models/Task.php) — truy van tasks theo user + filter, CRUD, `toggleComplete()`, `toggleImportant()`, dem task theo filter/list, thong ke.
...
- Views:
  - Layout: [app/Views/layout.php](app/Views/layout.php) — navbar, sidebar, flash, chen `$content` tu view con, tai CSS/JS.
  - Tasks: [app/Views/tasks/index.php](app/Views/tasks/index.php) render danh sach, nut toggle/important/delete, nut add task; [app/Views/tasks/create.php](app/Views/tasks/create.php) form tao task (chon list, due date, important).
